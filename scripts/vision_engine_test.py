...
def get_vision_data(sct, monitor):
    ...
    # 2. Monsters (Red/Orange) - Restored Perfect Geometric Logic
    ...
    # 3. Exits (Blue Squares) - ADDITIVE ULTRA BROAD TEST
    ...
    for cnt in contours_e:
        area = cv2.contourArea(cnt)
        # Broadest possible search for edge-case entries
        if 10 < area < 600:
            x, y, sw, sh = cv2.boundingRect(cnt)
            aspect_ratio = float(sw)/sh
            extent = float(area)/(sw*sh)
            # Very inclusive geometry
            if extent > 0.3 and 0.2 < aspect_ratio < 5.0:
